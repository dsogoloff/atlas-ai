...
import { submitInstructorUsefulnessCore } from "./instructorUsefulness";

// Persistence + access for the instructor usefulness rating. Uses injected
// mock clients (same DI seam as satisfaction.test.ts) so no live DB is needed.

const SESSION = "11111111-1111-4111-8111-111111111111";
const INSTRUCTOR = { id: "i1", tenant_id: "t1" };

afterEach(() => {
  vi.restoreAllMocks();
});

/** Chainable query-builder stub: select/eq return `this`; the terminal
...
function makeService() {
  ...
  const client = {
    from: vi.fn((table: string) => {
      if (table === "instructor_usefulness") return { upsert };
...
describe("submitInstructorUsefulnessCore", () => {
  it("persists the rating and emits the event for an accessible session", async () => {
    const { client, upsert, insert } = makeService();

    const result = await submitInstructorUsefulnessCore({
      rlsClient: makeRls(),
      serviceClient: client,
      instructor: INSTRUCTOR,
      input: { sessionId: SESSION, rating: 4, comment: "  helpful  " },
    });
    ...
    expect(payload).toMatchObject({
      ...
      session_id: SESSION,
      rating: 4,
      comment: "helpful", // trimmed
    });
    expect(opts).toEqual({ onConflict: "session_id,instructor_id" });

    // Event fired with rating + has_comment only — never the comment text.
    expect(insert).toHaveBeenCalledOnce();
    const eventRow = insert.mock.calls[0]![0];
    expect(eventRow.event_name).toBe("instructor_usefulness_submitted");
    expect(eventRow.props).toEqual({ rating: 4, has_comment: true });
    expect(JSON.stringify(eventRow)).not.toContain("helpful");
  });
  ...
  it("rejects when the child is not accessible to the instructor", async () => {
    const { client, upsert } = makeService();
    const result = await submitInstructorUsefulnessCore({
  ...
  it("rejects when the session is not visible to the instructor", async () => {
    const { client, upsert } = makeService();
    const result = await submitInstructorUsefulnessCore({
      rlsClient: makeRls({ session: { data: null, error: null } }),
      serviceClient: client,
      instructor: INSTRUCTOR,
      input: { sessionId: SESSION, rating: 3 },
    });
  ...
  it("surfaces a persistence failure as a soft error", async () => {
    ...
    (client.from as unknown as ReturnType<typeof vi.fn>).mockImplementation(
      (table: string) => {
        if (table === "instructor_usefulness")
