...
-- BEGIN l1-l4-short-eligible-backfill (lane/l1-l4-short-eligible-backfill)
-- MIRRORS supabase/migrations/20260619080000_l1_l4_short_eligible_backfill.sql
-- (dev/CI path). Sets short_test_eligible from each level doc's QUESTION SUMMARY
-- Short (Y/N) key; the stage4 loader predates the column so it defaults false.
-- Only short_test_eligible is written, only on the listed external_ids. Idempotent.
-- Key Short=Y tasks with no loaded row are omitted: L3-Q04, L3-Q23, L4-Q17.
...
-- L2 — Short=Y (21 of 22; only Q08 is N)
with t as (select id from tenants where slug = 'inspirea_singapore_math')
update questions q
set short_test_eligible = true
from t
...
-- L3 — Short=Y, existing rows (18; Q04/Q23 key Y but no DB row)
with t as (select id from tenants where slug = 'inspirea_singapore_math')
update questions q
set short_test_eligible = true
from t
...
-- L4 — Short=Y, existing rows (24; Q17 key Y but no DB row)
with t as (select id from tenants where slug = 'inspirea_singapore_math')
update questions q
set short_test_eligible = true
from t
