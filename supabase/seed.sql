...
-- BEGIN l0c-taxonomy-activation (lane/l0c-taxonomy-activation)
-- MIRRORS supabase/migrations/20260620140000_l0c_taxonomy_activation.sql (dev/CI).
-- New internal nodes l0c-geometry-4 "Comparing and Ordering" + l0c-whole_numbers-6
-- "Odd and Even Numbers"; activate Q05 (IMAGE_ORDERING, 3 carrot sizes), re-home
-- Q14 content_id, activate Q15 (SELECT_MULTIPLE odd numbers). Idempotent.
with t as (select id from tenants where slug = 'inspirea_singapore_math')
insert into tax_content (tenant_id, sub_strand_id, level_id, code, name, display_order, mvp)
select t.id,
  (select ss.id from tax_sub_strands ss where ss.tenant_id = t.id and ss.code = v.sub_strand_code),
