...
* ⚑ **The assessment-STARTED alert reuses the completion allowlist VERBATIM rather than widening it (PR #233, claude/staff-alert-assessment-start, 2026-08-20).** `AssessmentStartedAlert` is field-for-field identical to `AssessmentCompletedAlert` — parent name, child GRADE, staff record link. The brief's prose described the allowlist as "parent name / email / grade + admin link", but the actual completion alert carries no parent email; the narrower CODE definition won. Enforced two ways: an EXACT-EQUALITY assertion on the call-site payload in `sessionStart/handler.test.ts` (an extra field fails the test rather than shipping), plus the adversarial "careless caller" spread test in `notify.test.ts` asserting `placementBand` / `priorVersion` / `strandMastery` / `misconceptions` / child name / birth year never reach the wire. At start time no result exists even in principle, and none may be added later.

* **Verify allowlists against the CODE, never against a brief's prose description of them (2026-08-20).** The #233 brief mis-stated the #211 allowlist (see above) and only a code read caught it. This is now guardrail 7 in `CLAUDE.md`. Generalization: a brief is an instruction, not a source of truth about what the system currently does — read the typed interface and the test that pins it. The same failure class produced the false HubSpot portal id that propagated across two cycles.

* **The assessment-STARTED alert is wired to the fresh-session ANALYTICS seam, not to the session INSERT (PR #233, 2026-08-20).** It sits beside the `short_test_started` / `comprehensive_test_started` emit in `sessionStart/handler.ts`, which is the point where a session is both freshly created AND its first question served. That seam already carries the once-per-session guarantee for free: every re-entry route returns before it (resume-with-progress, zero-progress resume, the 23505 concurrent-start race loss, first-pick exhaustion rollback, consent block), and the partial unique index on `(child_id) WHERE status='IN_PROGRESS'` makes a second concurrent fresh session impossible. **The zero-progress resume is the subtle case and has its own test**: that response deliberately LOOKS fresh to the client (no resume banner, per the P3 fix) but must not re-alert. No bespoke once-per-session guard was added, for the same reason #211 rejected an atomic claim inside `closeSession`.

* **`/api/assess/start` resolves the canonical origin defensively, unlike `/api/assess/submit` (PR #233, 2026-08-20).** `getAppPublicOrigin()` THROWS when `APP_PUBLIC_ORIGIN` is unset or malformed (ATLAS-011 deliberately refuses a header fallback). On the submit route that surfaces as a 500, which is tolerable. On the START route it is not: a misconfigured env var must never stop a child from beginning an assessment for the sake of an internal courtesy email. The start route therefore catches it and passes `null`, degrading the student link to a bare path — the same fail-soft posture the notifier itself takes. The asymmetry is intentional and commented at both sites.

* **`CLAUDE.md` carries the standing guardrails, because repo context always loads and Drive context does not (PR #234, claude/claude-md-sam-os-wiring, 2026-08-20).** The SAM-OS `briefs/` STOP files are a warning a session might never read — they load only if the prompt names them. `CLAUDE.md` loads every session by construction. The seven standing prohibitions therefore live at the TOP of `CLAUDE.md`, above every other section, declared to outrank every brief, memory file and the rest of the document, with the conflict rule attached: a brief that contradicts a guardrail is **wrong, not newer** — stop, park a gate, report; and **do not triage a brief folder by filename, read bodies** (a prohibited build instruction has already appeared in this project buried in the standing-items section of a brief whose title advertised a benign UTM guardrail).

* **Uncommitted governance drift is COMMITTED via its own PR, never carried along in a feature PR and never silently restored (PR #234, 2026-08-20).** A `CLAUDE.md` modification had been sitting uncommitted in the main working tree across sessions, tripping the session-start drift alarm each time. It was deliberately excluded from #233 (mixing concerns) and given its own single-file PR, with the full diff shown to the founder before anything was committed. The decision recorded for INSP-ORCH: **committed, not restored** — the diff was legitimate SAM-OS wiring, not corruption. Corollary: after such a PR merges, the main working tree must be restored from the merged branch or it keeps reporting drift.

* **Push access is GRANTED; the earlier "stop on push failure" execution constraint is RETIRED (2026-08-20).** Prior sessions ran under an instruction to halt and file a BLOCKED handover if `git push` failed, precisely to prevent improvised workarounds (bundling the repo, committing to a different remote). `git push` to `origin` now works normally for `claude/*` branches and both #233 and #234 pushed and opened PRs without incident. `ATLAS-ASSESSMENT` remains protected and still rejects direct pushes; merge remains Dimitri's attended action after the Vercel preview. The anti-workaround rule survives the constraint: if a push ever fails again, stop and report — do not improvise.

## 2026-08-13

* **The #215 re-clamp backfill is CLOSED as NOT NEEDED — prod has no railed placements (verified 2026-08-13).** `pnpm backfill:reclamp:dry -- --target=prod` (read-only, zero writes) reported **0 would-re-clamp / 16 already-correct / 0 skipped**. A separate read-only enumeration of all 20 prod sessions confirmed the zero is a TRUE negative rather than a filter artifact: every one of the 16 completed sessions stores an `overall_level` exactly equal to its served ceiling, and the headline case — `a28f0c2a-c917-4877-9346-0cf430627f04`, served 0A×10 — reads **`0A` on prod, not `8B`**. The remembered 8B was local/dev data and has since been corrected. **The apply variant was deliberately NOT run**: executing a write path against a zero-row work list is risk with no benefit. The script and its README stay in-repo as a verified instrument should a railed session ever appear again.

* **Incidental confirmation that the #206 clamp is live in prod (2026-08-13).** Three of the four IN_PROGRESS prod sessions currently hold a provisional `1B` estimate against a served ceiling of `1A` (`263749ea…`, `2d3a2be6…`, `321e7b47…`) — the exact railing pattern. It is harmless because the clamp fires at FINALIZATION, so those sessions will be floored to `1A` when they complete. A mid-session estimate above the served ceiling is therefore expected and is NOT evidence of a defect; only a COMPLETED session above its ceiling would be.

* **Self-hosted webfonts — the production build must not depend on a third-party CDN (PR #217, lane/selfhost-fonts, 2026-08-13).** The production deploy of 486b230 failed with `pnpm run build` exit 1 on a commit whose only change was three `.agent/` markdown files; the commit 28 seconds earlier, carrying all of this session's code, deployed fine. Vercel log: `Received response with status 404 when requesting https://fonts.gstatic.com/s/plusjakartasans/v12/LDIWaomQ…woff2`, then `Module not found: Can't resolve '@vercel/turbopack-next/internal/font/google/font'` from `./src/app/layout.tsx`, `Turbopack build failed with 20 errors`. `next/font/google` downloads every requested weight from fonts.gstatic.com AT BUILD TIME (~21 static instances across four families); Google 404'd the URLs Next had resolved. Five latin VARIABLE woff2 (185 KB total) are now committed under `src/app/fonts/` and loaded via `next/font/local`, with all four SIL OFL 1.1 licenses alongside as the license requires. Axis ranges match the previously-requested weights and the subset is still latin, so rendering is unchanged; the built output contains zero `fonts.gstatic.com` references. `src/app/font-hosting.guard.test.ts` fails on any reintroduced `next/font/google` import, a missing woff2, or a dropped license, and asserts >50 files scanned so it cannot go vacuous (verified non-vacuous against the pre-fix `layout.tsx`). The Material Symbols `<link>` to fonts.googleapis.com is deliberately unchanged — it is a RUNTIME stylesheet, so an outage degrades icons on a deployed site rather than blocking a deploy.

* **Correction to a working assumption: the verify bar DOES build (2026-08-13).** `.github/workflows/verify.yml` has run `pnpm build` as its final step throughout, and that step was `success` on every lane of this session on ubuntu-latest. The 486b230 failure was therefore NOT a gap in the bar — the bar built the same code at a moment when Google served the files. Do not "fix" this class of failure by adding a build step that already exists; remove the external dependency instead. Related: two red Vercel statuses this session (25c56c4, 9774f91) were **"Canceled from the Vercel Dashboard"**, superseded builds from a rapid merge sequence, not failures — read the status DESCRIPTION before treating a red mark as a defect.
...
* **"#208 GA4 custom events not firing on prod" was NOT a code defect — root cause was GA4 report latency plus a dead debug channel (2026-08-11).** Verified on the live prod deploy, not from tests: the measurement id IS inlined in the prod bundle (`<link rel=preload …gtag/js?id=G-VEEHTL66LR>`); `window.gtag` is a function; `#ga4-loader` and the config both execute; `_ga`/`_ga_VEEHTL66LR` are set; no CSP is served. On `/report?child=…` the dedupe key `atlas:mkt:assessment_complete:<uuid>` was set AND `event|assessment_complete` was in `dataLayer`; clicking "Hand it over" at the handoff set `atlas:mkt:assessment_start:<uuid>` AND put `event|assessment_start` in `dataLayer` with the Meta half correctly queued. Both produced outbound `/g/collect` POSTs. The events were absent only from the standard **Event count by event name** report, which has 24–48h processing latency on a NEW event name and was read ~36h after deploy; the five events that DID appear are GA4 *automatically collected* events already accumulating since property creation, so their presence proved nothing. DebugView showing ZERO events (not "base but no custom") proves the Tag Assistant debug channel never attached — our config sets no `debug_mode` — so that evidence was void. Tag Assistant referral attribution affects source/medium only, never collection. **Two artifacts to not re-chase:** `503` on collect requests is an observation artifact (the known-good `page_view` hit returns 503 in the same observer), and DebugView is empty by default. Use Reports → **Realtime** to confirm collection.

* ⚑ **Analytics page context is REDACTED, because the #208 payload allowlist cannot see it (PR #210, lane/analytics-privacy-redaction, 2026-08-11).** GA4 sends `dl`/`dp`/`dr` and Meta sends `dl`/`rl` on every hit, read from the document — not from our payload. `/report?child=<uuid>` therefore handed Google and Meta a child identifier on every report pageview, straight past `buildEventPayload`'s allowlist; confirmed leaking on live prod with Meta returning HTTP 200 on both the automatic `PageView` and `assessment_complete`. `src/lib/marketing/redact.ts` is now the single pure boundary: it redacts identifier-bearing query values by KEY, any UUID-shaped value under ANY key (so a new route is covered without a code change), and UUID path segments (`/edit-child/<uuid>`); route shape and UTMs are preserved because that is what makes the hit useful; the fragment is dropped; it fails CLOSED on unparseable input. This is a privacy-boundary decision (strategy §2.5/§6.3) and must not be relaxed to "strip ?child=" only.

* **GA4 page context is applied with `gtag('set', …)` BEFORE `gtag('config', …)` — verified against the live tag, not assumed (2026-08-11).** Two findings that constrain any future change: (a) `set` works and applies globally, so the automatic `page_view` AND every later event inherit the redacted context, with no per-event plumbing for a new call site to forget; (b) a SECOND `gtag('config', ID, {page_location})` does **NOT** take effect once the tag is already configured — a config-only fix would have looked correct and silently left the leak in place. Because of this, the GA4 config now lives in typed code that can import `redact.ts` rather than in an inline `<Script>` string that cannot. `privacy-page-context.test.ts` pins the set-before-config ordering; do not reorder.

* ⚑ **The Meta Pixel is BLOCKED, not redacted, on any document whose URL carries a child identifier (PR #210, 2026-08-11).** Meta offers no supported way to override the URL it reports and reads the live document URL at send time, so redaction is impossible there and the only safe answer is not to send. Three gates on one predicate: the snippet is not injected, the deferred queue is not drained, and `track()` treats `fbq` as absent (that third gate is what covers a CLIENT-SIDE navigation into the report from a page where the pixel had already initialised — the first gate alone misses it). The event is queued and flushed from the next clean parent surface. **Accepted cost, needs founder ratification:** Meta's `assessment_complete` is deferred rather than immediate and Meta gets no report PageView. Privacy was chosen over conversion immediacy. Restoring immediacy requires the child id out of the query string — a routing change deliberately NOT taken in #210 because it risks colliding with existing report links; raised as a proposal.

* **GA4 given the same queue-and-replay durability Meta already had (PR #210, 2026-08-11).** `dispatchMarketingEvent` previously returned `"skipped"` when `window.gtag` was absent and the event was lost PERMANENTLY, while Meta queued and replayed. It survived only because `(parent)/layout.tsx` renders `<MarketingAnalytics/>` before `{children}`; the GA4 config is injected client-side post-hydration and is NOT in the SSR HTML, so a slow connection, an ad blocker delaying the tag, or a swap of those two JSX lines dropped `assessment_complete` with no error and no failing test. The queue core (TTL, cap, fail-soft parse, drain-before-send) moved to `event-queue.ts` and is shared by `meta-queue.ts` and the new `ga4-queue.ts`; both outcomes are now `"sent" | "queued"` and no path discards an event. `layout-order.test.ts` pins the island-before-children ordering in all four layouts so the queue stays a safety net rather than the normal path.

* **Diagnosing analytics on live prod writes real events; this is the accepted method but must be disclosed (2026-08-11).** The app-side test suite passed while the live path was the actual question, so live prod was treated as the source of truth (the known static-verifier-blind-spot pattern). Cost of this session's tracing: several probe `assessment_start`/`assessment_complete` hits plus `mechanism_probe*` events in the prod GA4 property, one real `assessment_complete` for report `1d9d158e…` and one real `assessment_start` for child `c284af8d…`. No assessment session was created (stopped before the child Welcome tap). The probe `atlas_attrib` cookie, the queued probe Meta event and the dedupe keys were all cleaned up afterwards. Rule going forward: prefer Realtime + `dataLayer` inspection over firing conversions, and always disclose what was emitted.
