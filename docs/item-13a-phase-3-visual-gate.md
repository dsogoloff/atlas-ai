...
- The cap is set via **inline `style={{ maxHeight: 'min(...)' }}`** on BOTH the wrapper `<div>` AND the `<img>`, not via Tailwind. The wrapper version is load-bearing — without it, the wrapper can grow tall (e.g. when an SVG with no intrinsic dimensions stretches to fill its flex slot) even if the img itself is correctly capped, and the wrapper's height is what pushes Submit below the fold. Tailwind v4 arbitrary values containing CSS `min()` commas (`max-h-[min(40vh,320px)]`) also turned out to be unreliable in practice; inline style is guaranteed.
- The placeholder SVG carries explicit `width="600" height="400"` attributes (not just `viewBox`) so browsers compute its intrinsic dimensions consistently. Without those, Chrome's SVG-as-img sizing fell back to fill-container in flex contexts, producing the original ~900px wrapper-height bug.
...
This section evolved through three gate findings on 2026-05-23. First attempt added vh-only height caps (40 vh / 30 vh) via Tailwind class; insufficient because on tall viewports (≥1000 px) the vh value exceeded the placeholder's 400 px natural height, so the cap never clamped. Second attempt combined the vh value with a pixel ceiling (320 px / 240 px) via Tailwind arbitrary class on the `<img>`; founder inspection showed the wrapper still measured ~900 px tall regardless, because (a) the Tailwind `max-h-[min(...)]` class with a `min()`-with-comma never made it onto the element, and (b) the wrapper had no cap of its own and the SVG (no intrinsic dimensions) was stretching to fill the flex slot. Third attempt (this one) moved both caps to inline `style`, added the cap to the wrapper as well as the img, and added explicit `width`/`height` to the SVG. Layout `items-center` → `items-start` change landed in the second attempt and remains.
