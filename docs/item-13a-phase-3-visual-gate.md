...
- The cap is set via **inline `style`** on BOTH the wrapper `<div>` AND the `<img>`, not via Tailwind, so all sizing rules live in one cascade layer and `min()`-with-comma JIT compilation isn't a failure mode.
- The wrapper version is load-bearing — without it, the wrapper can grow tall on its own and the wrapper's height is what pushes Submit below the fold.
- **Both elements also carry `min-height: 0`**, and the wrapper carries `flex: none` + `align-self: center`. This is the load-bearing fix from gate finding #3. The wrapper is a flex child of QuestionShell's content column (`flex flex-col items-center gap-10`). Flex items default to `min-height: auto` = intrinsic content height. Once the SVG carries explicit `width="600" height="400"` attributes (added in finding #2), that intrinsic height is 400 px — a 400 px FLOOR that overrides the 320 px CEILING (per CSS spec: when `min > max`, `max` is ignored). `min-height: 0` lifts that floor; `flex: none` stops the parent from re-stretching the row.
- The placeholder SVG carries explicit `width="600" height="400"` attributes (not just `viewBox`) so browsers compute its intrinsic dimensions consistently — required for `object-contain` to scale the bitmap proportionally inside the capped img box.

Layout companion fix (same commit as the first follow-up): the `<main>` element on QuestionShell.tsx switched from `items-center` to `items-start`, so when content exceeds main's available height the overflow goes downward only (the user scrolls to reach Submit) rather than splitting top-and-bottom (which previously hid both the top of the prompt AND the Submit button equally).
...
This section evolved through four gate findings on 2026-05-23. (1) First attempt added vh-only height caps (40 vh / 30 vh) via Tailwind class; on tall viewports (≥1000 px) the vh value exceeded the placeholder's 400 px natural height and never clamped. (2) Second attempt combined the vh value with a pixel ceiling (320 px / 240 px) via Tailwind arbitrary class on the `<img>` only; founder inspection showed the wrapper still measured ~900 px tall, because the Tailwind `min()`-with-comma arbitrary didn't compile and the wrapper had no cap of its own. (3) Third attempt moved both caps to inline `style`, added the cap to the wrapper as well as the img, and added explicit `width`/`height` to the SVG. Founder inspection then showed the inline `max-height` WAS reaching the DOM on the img but the image still rendered at intrinsic size. (4) Fourth attempt (this one) added `min-height: 0` to both wrapper and img, plus `flex: none` + `align-self: center` on the wrapper, to override the flex-child `min-height: auto` default that was setting a 400 px floor under the 320 px ceiling. The layout `items-center` → `items-start` change landed in attempt 2 and remains.
